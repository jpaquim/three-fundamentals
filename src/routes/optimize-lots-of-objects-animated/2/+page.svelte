<script lang="ts">
	...
	async function main() {
		...
		function dataMissingInAnySet(fileInfos: FileInfo[], latNdx: number, lonNdx: number) {
			for (const fileInfo of fileInfos) {
				if ((fileInfo.file as File).data[latNdx][lonNdx] === undefined) {
					return true;
				}
			}
			return false;
		...
		function makeBoxes(file: File, hueRange: [number, number], fileInfos: FileInfo[]) {
			...
			data.forEach((row, latNdx) => {
				row.forEach((value, lonNdx) => {
					...
					const amount = ((value as number) - min) / range;
		...
		async function loadAll() {
			...
			const geometries = fileInfos.map((info) => {
				return makeBoxes(info.file as File, info.hueRange, fileInfos);
			});
